...
# Example                                                            |
#--------------------------------------------------------------------+
#  | tian.command_parse.parse(outdir:str)
...
class command_parse:
    ...
    def parse(outdir:str, host_name:str, use_xterm, nolsf):
        command          = tian.command_parse.get_keys_from_command()
        option_from_file = tian.command_parse.get_option_infos()

        if (tian.command_parse.is_exist_key(command, option_from_file) == False) and (command != "genonly"):
            print(f"\nCommand: {sys.argv[1]}")
        ...
        tian.command_parse.match_option(command, option_from_file)
        print("")
        tian.command_parse.comment_key(option_from_file)
    ...
    def match_option(key:str, option_infos:list):
        ...
        if (re.match("^\s*[a-zA-Z].*$", key)):
            ...
            elif (tian.command_parse.is_exist_key(key, option_infos) == 0):
                print(f"*Error: {key} is not match with any regex")
                exit()
        else:
            tian.file.append("prun_execute.log", "    " + key + "\n")

        longest_opt = tian.command_parse.check_dupplicate_key(key, option_infos)
...
Result:
{body_str}""")
                for body_key in tian.command_parse.body_str2opt(body_str):
                    tian.command_parse.match_option(body_key, option_infos)
