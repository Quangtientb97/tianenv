...
if (len(sys.argv) == 1) or (tian.argv.has_argument("-h")):
    ...
    Run without bsub        : prun <command> -nolsf

    Syntax of mconf file:
        -prerun  <command> : command save in prerun  phase
        -postrun <command> : command save in postrun phase
        others:              save in run phase

        Example:
            run.mconf
                | run:
                |    -prerun echo "this is prerun"
                |    xrun -64 \\
                |        -uvm \\
                |        ...
                |    -postrun echo "this is postrun"
    """)
    exit()
...
print("")
command_parse.parse(outdir, host_name, use_xterm, nolsf)
